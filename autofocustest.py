#!/usr/bin/env python3
"""
Run this script after placing the FocusMetric implementation in
`focus_metric.py` (same directory) or adjust the import accordingly.
...
def main():
    # Initialize focus metric
    focus = FocusMetric(FocusConfig())
    is_preview = False  # Set to False to disable preview
    # Open default webcam
    cap = cv2.VideoCapture(0)
    cap.set(cv2.CAP_PROP_FPS, 30)  # Set FPS for webcam
    ...
    if is_preview:
        ...
        ax.set_ylabel("Focus value F = σx / σy")
        line, = ax.plot([], [], lw=2)
        ax.grid(True)

    times = deque(maxlen=500)   # keep last 500 points
    ...
    try:
        while True:
            ...
            result = focus.compute(frame)
            t_rel = result["t"] - t0
            F = result["focus"]

            print(f"Time: {t_rel:.2f}s, Focus value F: {F:.4f}")
            times.append(t_rel)
            values.append(F)
            
            if is_preview:
                # cut away history if too long 
                if len(times) > 100:
                    times.popleft()
                    values.popleft()
